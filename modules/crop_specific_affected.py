...
date_of_conflict_map = '2022-04-06'
...
class CropSpecificAffected:
    ...
    def oblast_conflict_df(self):
        df = pd.DataFrame(self.oblast_conflict_dict.values(), index = self.oblast_conflict_dict.keys(), columns = ['Under Conflict'])
        df.index.name = 'Oblast'
        return df
    ...
    def set_oblast_conflict_value(self, name, value):
        df = self.oblast_conflict_df.copy()
    ...
    def get_impact(self, year, crop, under_conflict_df: pd.DataFrame = None, data_path: Path= None):

        if under_conflict_df is None:
            under_conflict_df = self.oblast_conflict_df
        ...
        return group.apply(lambda x: ((x['Output']*x['Enterprise Share']*enterprise_share) + (x['Output']*(1-x['Enterprise Share'])*hh_share)), axis=1)*under_conflict_df['Under Conflict']

    def perc_impacted(self, year, crop, under_conflict_df: pd.DataFrame = None, data_path: Path = None):

        gi = self.get_impact(year, crop, under_conflict_df=under_conflict_df)
